...
BUTTONS = (("red", 28, "#943d48"), ("green", 244, "#28715c"),
           ("yellow", 460, "#80652d"), ("blue", 676, "#335e95"))
...
def make_skin():
    ...
    def label(name, x, y, w, h, font=22, color="#e9eff6", background="#101b2b"):
        return '<widget name="%s" %s font="Regular;%d" foregroundColor="%s" backgroundColor="%s" valign="center" />' % (
            name, rect(x, y, w, h), int(font * scale), color, background)

    widgets = [label("title", 28, 16, 844, 42, 30, "#57d4bd"),
               ...
               '<widget name="progress" %s borderWidth="1" foregroundColor="#57d4bd" backgroundColor="#203149" />' % rect(28, 352, 844, 10),
               label("metrics", 28, 370, 844, 28, 19, "#a8bbd0"),
               '<widget name="channels" %s itemHeight="%d" font="Regular;%d" scrollbarMode="showOnDemand" />' % (
    ...
    return '<screen name="NavigatorScreen" position="center,center" size="%d,%d" title="TKGS Navigator" backgroundColor="#101b2b">%s</screen>' % (
