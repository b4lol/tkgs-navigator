...
def main():
    ...
    for offset in range(0, 500, 50):
        payloads.append(b"".join(service_record(sid, "Demo Channel %d" % sid) + lcn_record(sid, sid)
                                 for sid in range(offset + 1, offset + 51)))
    sections = [section(payload, number=index, last=len(payloads) - 1)
    ...
    tracemalloc.stop()
    report = {"workload": "synthetic; not a satellite recording", "python": platform.python_version(),
              "machine": platform.machine(), "channels": 500, "sections": len(sections),
              "payload_bytes": sum(map(len, sections)), "iterations": len(samples),
              ...
              "collector_duplicates": collector.duplicates, "collector_peak_traced_bytes": peak}
