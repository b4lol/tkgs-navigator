...
def ar_bytes(members, epoch):
    ...
    for name, content in members:
        header = "{:<16}{:<12}{:<6}{:<6}{:<8}{:<10}`\n".format(
            name + "/", epoch, 0, 0, "100644", len(content)
        )
...
def build(output, libdir="usr/lib", epoch=0):
    ...
    files = {
        install + str(path.relative_to(ROOT / MODULE)): path.read_bytes()
        for pattern in ("*.py", "locale/*/LC_MESSAGES/*.mo")
        for path in (ROOT / MODULE).glob("**/" + pattern)
    ...
    artifacts = []
    for extension, dependency in (
        ("deb", "enigma2, python3 (>= 3.8)"),
...
if __name__ == "__main__":
    ...
    for artifact in build(
        arguments.output, arguments.libdir, int(os.environ.get("SOURCE_DATE_EPOCH", "0"))
    ):
        print(artifact)
